...
// Broker is a minimal NATS client that implements the broker interface.
type Broker struct {
	mu       sync.Mutex
	conn     net.Conn
	reader   *bufio.Reader
...
// New opens a NATS connection at url.
func New(url string) (*Broker, error) {
	...
	b := &Broker{
		...
		pingWait: make(map[string]chan error),
	}
...
func (b *Broker) handshake() error {
	if err := b.writeLine(`CONNECT {"verbose":false,"pedantic":false,"lang":"go","version":"v2","echo":false}`); err != nil {
		return err
	}
...
// Publish sends one message to the given topic.
...
// Subscribe registers a consumer for a topic and optional queue group.
...
// Health returns a minimal connection check.
