...
func (h Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == http.MethodGet && r.URL.Path == "/metrics":
		h.writeMetrics(w, r)
	case r.Method == http.MethodGet && r.URL.Path == "/relay/v1/data/validator_registration":
		h.writeValidatorRegistration(w, r)
	case r.Method == http.MethodGet && r.URL.Path == "/relay/v1/builder/validators":
		h.writeBuilderValidators(w, r)
	case r.Method == http.MethodPost && (r.URL.Path == "/relay/v1/builder/blocks" || r.URL.Path == "/relay/v3/bundle"):
		h.submitBundle(w, r)
	case r.Method == http.MethodGet && strings.HasPrefix(r.URL.Path, "/relay/v1/builder/blocks"):
		h.getBundle(w, r)
	default:
		http.NotFound(w, r)
	}
...
func (h Handler) writeValidatorRegistration(w http.ResponseWriter, r *http.Request) {
	h.writeJSON(w, map[string]any{
		"regionId": h.Svc.cfg.RegionID,
		"shardId":  h.Svc.cfg.ShardID,
		"shards":   h.Svc.cfg.ShardSet,
		"state":    h.Svc.AssessHealth(r.Context()).State,
	})
}

func (h Handler) writeBuilderValidators(w http.ResponseWriter, r *http.Request) {
	h.writeJSON(w, map[string]any{
		"shardId": h.Svc.cfg.ShardID,
		"regionId": h.Svc.cfg.RegionID,
	})
