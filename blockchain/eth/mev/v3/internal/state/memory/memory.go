...
type Store struct {
	...
	authority   map[string]graph.Authority
	limit       int
	closed      bool
}
...
func (s *Store) CreateBundle(ctx context.Context, rec model.BundleRecord) (model.BundleRecord, error) {
	...
	if err := s.validateAuthorityLocked(rec); err != nil {
...
func (s *Store) validateAuthorityLocked(rec model.BundleRecord) error {
	if rec.ShardID == "" {
		return nil
	}
	auth, ok := s.authority[rec.ShardID]
	if !ok {
